...
class JournalMatrixView(generics.GenericAPIView):
    ...
    def get(self, request, format=None):
        ...
        journal_matrix_serializer = JournalMatrixSerializers(
            journal_matrix_obj, many=True
        )
    ...
    def post(self, request, format=None):
        ...
        if journal_matrix_serializer.is_valid():
            ...
            return Response(
                journal_matrix_serializer.data, status=status.HTTP_201_CREATED
            )
        return Response(
            journal_matrix_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
